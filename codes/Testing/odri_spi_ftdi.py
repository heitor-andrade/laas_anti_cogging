...
class SPIuDriver:
  def __init__(self, waitForInit = True, absolutePositionMode = False, offsets=[0.0,0.0]):
    ...
    self.ctrl.configure('ftdi://ftdi:232h:01:05/1') 
  ...
  def transfer(self):
    ...
    rawRefIq0 = int(self.refCurrent0*(1<<8))
    ...
    rawKp0 = int(self.kp0*(1<<8))
    rawKp1 = int(self.kp1*(1<<8))
    rawKd0 = int(self.kd0*(1<<8))
    rawKd1 = int(self.kd1*(1<<8))
    ...
    #decode received sensor packet
      ...
      self.supply0 = data[10]  / (1<<14)
      self.supply1 = data[11]  / (1<<14)
