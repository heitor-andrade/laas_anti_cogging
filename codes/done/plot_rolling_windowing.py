...
times = [times[i]%(period) for i in range(len(times))]
# Limit position range in [0, 2pi]
positions = [positions[i]%(2*pi) for i in range(len(positions))]
...
# Tamanho da amostra do array
N = len(times)
tukey = signal.windows.tukey(N, alpha=0.2)

# Aplique a janela ao sinal
windowed_signal = (np.array(fs) - a0) * tukey

fs = list(windowed_signal + a0)
...
# Plot results
plt.plot(times, velocities, label = "Velocity Data")
plt.plot(xs, fs, label = "Windowing Data")
