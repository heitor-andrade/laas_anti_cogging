...
fs = uniform_currents           # f(x): Anti-cogging current
...
dif_fourier = [abs(fourier[i] - fourier[i+1]) for i in range(len(fourier))]
plt.plot(fourier_xs[:-1], dif_fourier, '.' ,label = "Fourier Function")
plt.title(f"Diference between point of Anti-cogging")
