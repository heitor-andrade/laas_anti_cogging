...
NUM_POINTS = 360*2
uniform_times = np.linspace(0, period, NUM_POINTS)
uniform_velocities = []
uniform_positions = []
...
for i in range(len(uniform_times)):
    temp_vs = []
    temp_ps = []
    left_lim = uniform_times[i-1]  
    if i != len(uniform_times) - 1:
        right_lim = uniform_times[i+1]        
    else:
        right_lim = uniform_times[1]

    if i == 0 or i == len(uniform_times) - 1:
        for j in range(len(times)):
            if times[j] > left_lim or times[j] < right_lim:
                temp_vs.append(velocities[j])
                temp_ps.append(positions[j])

    else: # if 0 < i < len(pos) - 1
        for j in range(len(times)):
            if times[j] > left_lim and times[j] < right_lim:
                temp_vs.append(velocities[j])
                temp_ps.append(positions[j])
    
    uniform_velocities.append(sum(temp_vs) / len(temp_vs))
    uniform_positions.append(sum(temp_ps) / len(temp_ps))
...
plt.title(f"Velocity fit with uniform sampling in time")
plt.xlabel("Time [s]")
plt.ylabel("Velocity [rad/s]")
plt.grid()
plt.legend()
...
acc2cur = 0.000196
iqs_cogging = np.array(acs)*acc2cur*(-1)

plt.plot(ps, iqs_cogging, '.')
