...
inline float32_t FOC_runPI(pi_t*, float);
...
inline float AntiCogging(foc_t* p_foc)
{
    // add anti-cogging_current
 ...
 */
inline float32_t FOC_runPI(pi_t* p_pi, float anti_cogging_current)
{
    float32_t sat_max   = 0.5f  * (*p_pi->p_sat);
    float32_t sat_min   = -1.0f * sat_max;
    p_pi->err           = (*p_pi->p_set) + anti_cogging_current - (*p_pi->p_fb);
